...
# just remove the multi-stemmed individuals
multi_ID <- LAU_only5[which(LAU_only5$mstem > 0), 'treeID']
# 16127 before
LAU_only5 <- LAU_only5[-which(LAU_only5$treeID %in% multi_ID), ]
# 11821 after, but broken
stop('cant just drop multi-stemmed, need better method')
# for all the treeID in which mstem = 0 & DFstatus == 'dead', what is DFstatus of other stems?
# if DFstatus of other stems can be a table, what is a table of whether that table is all 100% 'dead' or < 100% dead?
```
...
```{r process_2, echo = F}
# could multiple stems be confounding the data?
```
